...
public final class Gestionar_Factura extends javax.swing.JInternalFrame {
    ...
    private void initComponents() {
        ...
        btnRefrescar.setBackground(new java.awt.Color(188, 221, 255));
...
//        DC.ver_documento("Factura", Integer.parseInt(lblid.getText()));
    ...
    private void btnuptSCActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnuptSCActionPerformed
        ...
        if (opc == 0) {
            actualizar_docs_cont(3, Integer.parseInt(lblid.getText()), "SOPORTE DE CAUSACION");
        } else {
    ...
    private void btnuptDQActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnuptDQActionPerformed
        ...
        if (opc == 0) {
            actualizar_docs_cont(4, Integer.parseInt(lblid.getText()), "DOCUMENTO EQUIVALENTE");
        } else {
            btnRefrescar.setEnabled(true);
    ...
    private void btnuptNPActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnuptNPActionPerformed
        ...
        if (opc == 0) {
            actualizar_docs_cont(7, Integer.parseInt(lblid.getText()), "NOTA A PROVEEDOR");
        } else {
            btnRefrescar.setEnabled(true);
    ...
    private void btnuptNIActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnuptNIActionPerformed
        ...
        if (opc == 0) {
            actualizar_docs_cont(6, Integer.parseInt(lblid.getText()), "NOTA INTERNA");
        } else {
            btnRefrescar.setEnabled(true);
    ...
    private void btnuptSUActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnuptSUActionPerformed
        ...
        if (opc == 0) {
            actualizar_docs_cont(10, Integer.parseInt(lblid.getText()), "SOPORTE ANTICIPO");
        } else {
            btnRefrescar.setEnabled(true);
    ...
    private void btnrechazarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnrechazarActionPerformed
        ...
        int id_factura, id_proveedor, id_empresa, id_usuario, id_estado, est_prev;
        ...
        est_prev = TC.est_prev(id_factura);
        Rechazo_Factura RF = new Rechazo_Factura();
        Principal.Escritorio.add(RF);
        RF.Rechazo_Factura(id_usuario, id_factura, est_prev, 11, id_proveedor, id_empresa, no_factura);
    ...
    public void enviar_a_aprobacion() {
        if (lblnombreu.isVisible() && !btnaprobar.isVisible()) {
            ...
            if (FC.cambiar_asignacion_factura(no_factura, id_proveedor, id_empresa, 2, 1)) {
                ...
                registro_procedimiento(no_factura, id_factura, id_usuario, 8, 2, tacomentario.getText(), 1, 0, "Factura pendiente de Aprobar", 1);
    ...
    public void enviar_a_contabilidad() {
        ...
        if (signature.firmar(DC.ubicacion_documento("Factura", id_factura), fechaactual.format(fecha), horaactual.format(fecha))) {
            ...
            NS.notificaciones("Aprobación Factura", "La Factura '" + lblnfact.getText() + "' no pudo ser firmada debido a que el documento se encuentra abierto.\n"
                    + "Por favor validar que el documento se encuentre cerrado y esperar 15 segundos.", "error firma");
        }
    }
    ...
    public void enviar_a_revision() {
        if (cmbRcont.getSelectedIndex() == 0) {
            ...
            if (FC.cambiar_asignacion_factura(no_factura, id_proveedor, id_empresa, 13, 5)) {
                ...
                registro_procedimiento(no_factura, id_factura, id_usuario, 4, 13, tacomentario1.getText(), 5, id_empresa, "Factura por Revisar", 1);
    ...
    public boolean firmar_rev() {
        ...
        } else if (DC.existe_documento("Documento Equivalente", id_factura)) {
            return signature.firmar_rev(DC.ubicacion_documento("Documento Equivalente", id_factura), fechaactual.format(fecha), horaactual.format(fecha));
    ...
    public void enviar_a_tesoreria() {
        ...
        if (firmar_rev()) {
            if (FC.cambiar_asignacion_factura(no_factura, id_proveedor, id_empresa, 9, 9)) {
                ...
                registro_procedimiento(no_factura, id_factura, id_usuario, 13, 9, tacomentario3.getText(), 9, id_empresa, "Factura Pendiente de Pago", 1);
                Principal.btnactualizar.doClick();
    ...
    public void confirmar_pago() {
        ...
        if (!DC.existe_documento("Comprobante de Pago", id_factura)) {
            ...
            if (FC.confirmacion_pago(no_factura, id_proveedor, id_empresa, 7, 9, fecha_pago)) {
                if (est_prev == 10) {
    ...
    public void programar_factura() {
        ...
        if (ProgC.crear_programacion(id_factura, id_usuario, fecha_prog)) {
            if (FC.cambiar_estado_factura(lblnfact.getText(), id_proveedor, id_empresa, 10)) {
                ...
                Principal.btnactualizar.doClick();
                NS.notificaciones("Programación Factura", "La Factura " + lblnfact.getText() + " ha sido programada.", "correcto");
                this.doDefaultCloseAction();
            } else {
    ...
    public void retirar_factura() {
        String no_factura, com, proveedor, empresa, ruta;
        int id_factura, id_proveedor, id_empresa, id_usuario;
        no_factura = lblnfact.getText();
        proveedor = lblproveedor.getText();
        empresa = lblempresa.getText();
        id_factura = Integer.parseInt(lblid.getText());
        ruta = DC.ubicacion_documento("Factura", id_factura);
        id_proveedor = PC.id_proveedor(proveedor);
        id_empresa = EMPC.id_empresa(empresa);
        id_usuario = UC.id_usuario(Principal.lbluser.getText());
        int opc = JOptionPane.showConfirmDialog(Principal.Escritorio, "¿Confirma que desea retirar la factura?",
                "Retirar Factura", JOptionPane.YES_NO_OPTION);
        if (opc == 0) {
            com = JOptionPane.showInputDialog(Principal.Escritorio,
                    "<html><div style='font-size: 16px;'>Por favor indique el motivo del retiro de la factura</div></html>",
                    "Motivo Retiro de Factura",
            ...
            if (com.equals("")) {
                ...
                if (FC.cambiar_asignacion_factura(no_factura, id_proveedor, id_empresa, 1, 1)) {
                    ...
                    registro_procedimiento(no_factura, id_factura, id_usuario, 12, 12, com, 1, id_empresa, "", 1);
                    Principal.btnactualizar.doClick();
                    NS.notificaciones("Retirar Factura", "Se realiza el retiro de la factura  " + lblnfact.getText() + ".", "correcto");
                    this.doDefaultCloseAction();
                } else {
                    NS.notificaciones("Retirar Factura", "No se ha podido retirar la factura  " + lblnfact.getText() + ".", "error");
                }
            }
        }
    }
        ...
        if (cor == 1) {
            if (id_area_dest == 4) {
                CorC.crear_correo(id_factura, UC.correos(id_area_dest, id_empresa), asunto, EC.plantilla_correo(asunto,
                        ...
                        lblvalor.getText(), lblfecharec.getText(), lblfechafact.getText(), lblfechavenc.getText(), usuario, comentario));
            } else {
                if (asunto.equals("Factura por Revizar")) {
                    CorC.crear_correo(id_factura, RC.correo_usuario_rev(id_factura), asunto, EC.plantilla_correo(asunto,
                            no_factura, lblempresa.getText(), lblnit.getText(), lblcorresp.getText(), lblproveedor.getText(),
                            lblvalor.getText(), lblfecharec.getText(), lblfechafact.getText(), lblfechavenc.getText(), usuario, comentario));
                } else {
    ...
    public void componentes_visibles(String area, String estado) {
        ...
        switch (area) {
            ...
            case "Contabilidad_Rev":
                ...
                btnctdoc.setVisible(false);
                facturas_anticipadas(2);
                switch (UC.id_usuario(Principal.lbluser.getText())) {
                    case 21:
                        btnctdoc.setVisible(true);
                        break;
                    default:
